...
- 编辑模式
  ...
  - 桌面端支持导入实机截图：选择本机图片后复制到 `assets/screenshots/<map-id>/`，并自动填写资源相对路径。
  - 导入截图在保存当前地图配置前处于待确认状态；取消编辑、正常退出或异常退出后的下次启动会清理未保存导入。
  - 浏览器预览不能导入截图，可手动填写 `public/assets/screenshots/<map-id>/` 下对应资源路径。
  - 支持删除点位。
  - 支持拖拽移动点位。
  - 新增点位时截图可以为空。
